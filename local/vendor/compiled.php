...
class Carbon extends DateTime
{
    ...
    public static function setLocale($locale)
    {
        ...
        if (file_exists($filename = 'C:\\wamp64\\www\\viralfb\\local\\vendor\\nesbot\\carbon\\src\\Carbon' . '/Lang/' . $locale . '.php')) {
...
class AppServiceProvider extends ServiceProvider
{
    public function boot()
    {
        if (file_exists(storage_path('installed'))) {
            $json_cu_variabile = DB::table('configs')->where('nume', 'basic')->pluck('valoare');
            $json_cu_variabile = json_decode($json_cu_variabile, true);
            $json_cu_culori = DB::table('configs')->where('nume', 'colors')->pluck('valoare');
            ...
            $orig = array('[site-domain]', '[year]', '[url]');
            $new = array(Request::server('SERVER_NAME'), date('Y'), url());
    ...
    public function register()
    {
        $this->app->bind('Illuminate\\Contracts\\Auth\\Registrar', 'viralfb\\Services\\Registrar');
    }
}
}
...
class BusServiceProvider extends ServiceProvider
{
    public function boot(Dispatcher $dispatcher)
    {
        $dispatcher->mapUsing(function ($command) {
            return Dispatcher::simpleMapping($command, 'viralfb\\Commands', 'viralfb\\Handlers\\Commands');
        });
    }
    public function register()
    {
